...
$email = $_POST['email'];
$type = 'solider'; // new member addition only for solider


$query= "SELECT * FROM tbl_users_214 WHERE email='$email' OR identity_number='$id_num'";
$result = mysqli_query($connection, $query);
if (!$result->num_rows > 0) { 								
  $query = "INSERT INTO tbl_users_214 (username, email, birth_day ,identity_number, type)
      VALUES ('$fname', '$email', '$birth_date' , '$id_num' , '$type')";
   echo "<script>alert('Added successfully to Myteam!');document.location='connect-list.php?Id=" . $_GET['Id'] . "&Name=" . $_GET['Name'] . "'</script>";  
  $result = mysqli_query($connection, $query);
  if ($result) {
    $username = "";
    $email = "";
...
<html lang = "en">
    ...
    <div class="container">
      ...
      <nav id="nav-menu-container">
        <ul class="nav-menu">
          ...
          echo '<li class="menu-has-children"><a href="connect-list.php?Id='. $_GET['Id'] .'&Name=' . $_GET['Name'] .'&type=' . $_GET['type'] .'">My team</a>
            <ul>
              <li class="menu-active"><a href="#">Add a team member</a></li>
      ...
      <nav aria-label="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="home.php?Id=<?php echo $_GET['Id']; ?>&Name=<?php echo $_GET['Name']?>&type=<?php echo $_GET['type']?>">Home</a></li>
          ...
          <li class="breadcrumb-item active" aria-current="page">Add a new team member</li>
        </ol>
      </nav>
 ...
 <form action="" method="POST" id=add_new_form>
  ...
  <p><input type="text" class="form-control" name="fname"></p>
  
  <label for="identityNum">Identity number:</label> 
  <p><input type="number" name="id_num" pattern="[0-9]{7}" inputmode="numeric" aria-describedby="numberHelpInline" required>
    <span id="numberHelpInline" >Must be 7 numbers long</span></p> 
  ...
  <button type="submit" class="btn btn-light" name="submit">Add to my team</button>
